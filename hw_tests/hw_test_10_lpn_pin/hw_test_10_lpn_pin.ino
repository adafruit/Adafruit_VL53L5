 ...
 *
 * Hardware test: VL53L5CX LPn (reset/enable) pin
 *
...
void setup() {
  ...
  bool reinitOk = vl53l5cx.begin(0x29, &HW_TEST_WIRE);
  ...
  if (reinitOk) {
    ...
    while (millis() - t < 3000) {
      if (vl53l5cx.isDataReady()) {
        ...
        gotData = vl53l5cx.getRangingData(&results);
        if (gotData) {
